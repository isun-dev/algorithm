...
# 접근 방향
1. for문을 돌리면서 arr에 있는 값들이, divisor로 나누어지는지 확인한다.
2. 나누어지는 값이 몇개가 있을지 모르니까 ArrayList를 만든다.
3. 나누어지면 ArrayList 에 추가한다.
4. 나누어지는게 없으면 ArrayList의 size를 확인하여 -1를 리턴한다.
5. 나누어지는게 있으면 ARRAY.SORT를 이용해서 오름차 순으로 정렬해서 응답한다.

## 보충포인트
- Arrays.sort()는 배열(int[])에서만 사용한다.
- ArrayList를 정렬할때는 Collections.sort를 쓴다.
