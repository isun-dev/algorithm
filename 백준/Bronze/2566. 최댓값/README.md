...
## 문제풀때 주의점
- 해당 문제는 시간제한이 1초이다. 
- 그렇기 때문에 Scanner를 사용하면 로직은 맞아도 속도 제한에서 "틀렸습니다!"라는 결과가 난다.
- 그리고 최대값이 있는 인덱스는 if문안에서 연산을 하지 말고 i,j 인덱스만 변수에 저장을 해두고, 출력할 때만 연산을 하는 것이
속도향상 측면에서 더 효율적이다.
- 속도를 무조건 고려해야 한다. 그렇지 않으면 틀린다!
